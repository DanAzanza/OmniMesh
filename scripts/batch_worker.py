...
blender.exe -b <blend_file> --factory-startup --disable-autoexec --python scripts/batch_worker.py -- <args>
"""
...
def setup_omnimesh_path() -> None:
    ...
    # current_file is <addon_root>/scripts/batch_worker.py -> addon_root is parent.parent
...
def run_worker() -> int:
    ...
    if not getattr(getattr(bpy, "app", None), "background", False):
        logger.error("Batch worker must be run in background/headless mode (-b).")
        return 2
